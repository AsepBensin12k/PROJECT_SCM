        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex flex-col">
                        <a href="{{ route('koordinator.dashboard') }}"
                            class="text-white font-bold text-lg tracking-tight hover:text-amber-100 transition duration-200">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <a href="{{ route('koordinator.dashboard') }}"
                        class="flex items-center space-x-2 bg-white/20 px-4 py-2 rounded-xl border border-white/30 hover:bg-white/30 transition-all duration-200 group">
                        <i
                            class="fas fa-arrow-left text-white text-sm group-hover:-translate-x-1 transition-transform duration-200"></i>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($distributions as $distribution)
                                <tr class="hover:bg-gray-50 transition duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $distribution->product->name }}</div>
                                        <div class="text-sm text-gray-500">Harga: Rp
                                            {{ number_format($distribution->product->price, 0, ',', '.') }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $distribution->destination }}</div>
                                        @if ($distribution->notes)
                                            <div class="text-sm text-gray-500">
                                                {{ Str::limit($distribution->notes, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <span
                                            class="px-2 py-1 text-xs font-medium {{ $statusColors[$distribution->status] }} rounded-full">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-2">
                                            ...
                                            <form
                                                action="{{ route('koordinator.distributions.destroy', $distribution->id) }}"
            ...
            <div class="mt-8 grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-green-500">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-sm font-medium text-gray-600">Total Barang Dikirim</h3>
                            <p class="text-2xl font-bold text-gray-800">{{ $totalQuantitySent }} pcs</p>
                        </div>
                    </div>
                </div>
